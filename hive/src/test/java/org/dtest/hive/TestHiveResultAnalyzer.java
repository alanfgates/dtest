...
public class TestHiveResultAnalyzer {
  ...
  public void unitTestWithFailures() throws IOException  {
    ...
    Assert.assertEquals(1, cr.getLogFilesToFetch().size());
    Assert.assertTrue(cr.getLogFilesToFetch().containsKey("TestSchemaToolForMetastore"));
    SortedSet<String> orderedLogFiles = new TreeSet<>();
    for (List<String> logFiles : cr.getLogFilesToFetch().values()) orderedLogFiles.addAll(logFiles);
  ...
  public void unitTestWithFailuresBranch2() throws IOException  {
    ...
    Assert.assertEquals(BuildState.State.HAD_FAILURES_OR_ERRORS, analyzer.getBuildState().getState());
    Assert.assertEquals(1, cr.getLogFilesToFetch().size());
    Assert.assertTrue(cr.getLogFilesToFetch().containsKey("TestTxnCommands2WithSplitUpdateAndVectorization"));
    SortedSet<String> orderedLogFiles = new TreeSet<>();
    for (List<String> logFiles : cr.getLogFilesToFetch().values()) orderedLogFiles.addAll(logFiles);
    Iterator iter = orderedLogFiles.iterator();
  ...
  public void qtestLogWithFailures() throws IOException {
    ...
    Assert.assertEquals(BuildState.State.HAD_FAILURES_OR_ERRORS, analyzer.getBuildState().getState());
    Assert.assertEquals(1, cr.getLogFilesToFetch().size());
    Assert.assertTrue(cr.getLogFilesToFetch().containsKey("TestCliDriver"));
    SortedSet<String> orderedLogFiles = new TreeSet<>();
    for (List<String> logFiles : cr.getLogFilesToFetch().values()) orderedLogFiles.addAll(logFiles);
    Iterator iter = orderedLogFiles.iterator();
  ...
  public void qtestLogWithFailuresBranch2() throws IOException {
    ...
    Assert.assertEquals(BuildState.State.HAD_FAILURES_OR_ERRORS, analyzer.getBuildState().getState());
    Assert.assertEquals(1, cr.getLogFilesToFetch().size());
    Assert.assertTrue(cr.getLogFilesToFetch().containsKey("TestMiniLlapCliDriver"));
    SortedSet<String> orderedLogFiles = new TreeSet<>();
    for (List<String> logFiles : cr.getLogFilesToFetch().values()) orderedLogFiles.addAll(logFiles);
    Iterator iter = orderedLogFiles.iterator();
